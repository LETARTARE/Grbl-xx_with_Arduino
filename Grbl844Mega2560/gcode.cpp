      ...
      case 'G':
        ...
        switch(int_value) {
          case 4: case 10: case 28: case 30: case 53: case 92:
            group_number = MODAL_GROUP_0;
            break;
          case 0: case 1: case 2: case 3: case 80:
            group_number = MODAL_GROUP_1;
            break;
          case 17: case 18: case 19:
            group_number = MODAL_GROUP_2;
            break;
          case 90: case 91:
            group_number = MODAL_GROUP_3;
            break;
          case 93: case 94:
            group_number = MODAL_GROUP_5;
            break;
          case 20: case 21:
            group_number = MODAL_GROUP_6;
            break;
          case 54: case 55: case 56: case 57: case 58: case 59:
            group_number = MODAL_GROUP_12;
            break;
        }
        // Set 'G' commands
        switch(int_value) {
      ...
      case 'M':
        ...
        switch(int_value) {
          case 0: case 1: case 2: case 30:
            group_number = MODAL_GROUP_4;
            break;
          case 3: case 4: case 5:
            group_number = MODAL_GROUP_7;
            break;
        }
        // Set 'M' commands
        switch(int_value) {
	    ...
	    #ifdef ENABLE_M7
          case 7:
            gc.coolant_mode = COOLANT_MIST_ENABLE;
            break;
	    #endif
          case 8:
            gc.coolant_mode = COOLANT_FLOOD_ENABLE;
            break;
	      case 9:
            gc.coolant_mode = COOLANT_DISABLE;
            break;
          default: {
            FAIL(STATUS_UNSUPPORTED_STATEMENT);
            }
...
/// <=
      case 'F':
        if (value <= 0) {
            FAIL(STATUS_INVALID_COMMAND); // Must be greater than zero
        }
      ...
      case 'S':
        if (value < 0) {
            FAIL(STATUS_INVALID_COMMAND); // Cannot be negative
        }
        gc.spindle_speed = value;
        break;
      case 'T':   // Choice of tool
        if (value < 0) {
            FAIL(STATUS_INVALID_COMMAND);  // Cannot be negative
        }
        gc.tool = trunc(value);
...
/// TODO : 842 : to_millimeters ==> to_degrees
  ...
  switch (non_modal_action) {
    ...
    /// -> 843 : 4 -> N_AXIS
        for (i=0; i<N_AXIS; i++) { // Axes indices are consistent, so loop may be used.
          if ( bit_istrue(axis_words,bit(i)) )
            sys.coord_system[int_value][i] = target[i];
        }
      }
    ...
    case NON_MODAL_GO_HOME:
     ...
     /// -> 843 : 4 -> N_AXIS
        ...
        mc_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[T_AXIS], settings.default_seek_rate, false);
      }
      mc_go_home();
 ...
 /// -> 843 : 4 -> N_AXIS
    ...
    switch (gc.motion_mode) {
      case MOTION_MODE_CANCEL:
        if (axis_words)  {
            FAIL(STATUS_INVALID_COMMAND);  // No axis words allowed while active.
        }
        break;
  ...
  if (gc.program_flow) {
    ...
    if (gc.program_flow == PROGRAM_FLOW_COMPLETED)
        sys.abort = true;
  }
