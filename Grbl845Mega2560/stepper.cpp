...
/// 844 : axis T
    st.counter_t += current_block->steps_t;
	if (st.counter_t > 0) {
      out_bits |= (1<<T_STEP_BIT);
      st.counter_t -= st.event_count;
      if (out_bits & (1<<T_DIRECTION_BIT)) {
		sys.position[T_AXIS]--;
      }
      else {
		sys.position[T_AXIS]++;
...
/// LETARTARE 20/04/2014 : comparison between signed and unsigned integer expressions
/// step_events_completed (uint32_t), step_event_count (int32_t)
