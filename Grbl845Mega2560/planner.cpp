...
/// 843  : x, y, and z -> x, y, z and a
// Add a new linear movement to the buffer. x, y, z and a is the signed, absolute target position in
...
void plan_buffer_line(double x, double y, double z, double t, double feed_rate, uint8_t invert_feed_rate)
{
...
#else
  target[T_AXIS] = lround(t*settings.steps_per_degree);
#endif
...
#else
  delta_mm[T_AXIS] = (target[T_AXIS]-pl.position[T_AXIS])/settings.steps_per_degree;
#endif

  block->millimeters = sqrt(delta_mm[X_AXIS]*delta_mm[X_AXIS] + delta_mm[Y_AXIS]*delta_mm[Y_AXIS] +
                            delta_mm[Z_AXIS]*delta_mm[Z_AXIS] + delta_mm[T_AXIS]*delta_mm[T_AXIS]);
