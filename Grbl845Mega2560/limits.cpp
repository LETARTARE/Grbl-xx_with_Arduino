...
static void homing_cycle(bool x_axis, bool y_axis, bool z_axis, bool t_axis, int8_t pos_dir, double homing_rate)
{
...
/// 844
	...
	steps[T_AXIS] = lround(settings.steps_per_degree);
#else
...
/// 844 : axis T
double ds = step_event_count/sqrt(x_axis + y_axis + z_axis + t_axis);
...
// 844 : axis T
    if (t_axis) {
      counter_t += steps[T_AXIS] ;
      if (counter_t > 0) {
        if (LIMIT_PIN & (1<<T_LIMIT_BIT))  { out_bits ^= (1<<T_STEP_BIT); }
        else { t_axis = false; }
    ...
    if (!(x_axis || y_axis || z_axis || t_axis) || sys.abort) { return; }
...
static void approach_limit_switch(bool x, bool y, bool z, bool a)
{
  homing_cycle(x, y, z, a, true, settings.default_seek_rate);
}


static void leave_limit_switch(bool x, bool y, bool z, bool a) {
  homing_cycle(x, y, z, a, false, settings.default_feed_rate);
}
...
void limits_go_home()
{
  ...
  approach_limit_switch(false, false, false, true);  // Then home the a axis
  ...
  leave_limit_switch(
		...
		limit_switches_present & (1<<T_LIMIT_BIT)
		);
  delay_ms(LIMIT_DEBOUNCE); // Delay to debounce signal before leaving limit switches
}
